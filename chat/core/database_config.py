...
class QdrantConfig(BaseModel):
    ...
    url: Optional[str] = Field(
        default=None,
        description="Qdrant 云服务器 URL"
    )
    api_key: Optional[str] = Field(
        default=None,
        description="Qdrant 云服务器 API Key"
    )

    # 集合配置
    collection_name: str = Field(
        default="memory_collection",
    ...
    distance: str = Field(
        default="cosine",
        description="向量距离度量方式，可选 'cosine', 'euclidean', 'dot'"
    )

    # 连接配置
    timeout: int = Field(
        default=30,
        description="连接 Qdrant 服务器的超时时间（秒）"
    )
    ...
    def from_env(cls) -> "QdrantConfig":
        return cls(
            ...
            collection_name=os.getenv("QDRANT_COLLECTION_NAME", "memory_collection"),
